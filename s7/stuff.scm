...
(define-macro (nested-define name . body)       ; from srfi-201/219
  (define (define-1 decl . body)
    (if (symbol? decl)
	`(define ,decl ,@body)                  ; (define f 32)
	(if (pair? (car decl))                  ; (define ((f...) ...)...)
	    (define-1 (car decl) (cons 'lambda (cons (cdr decl) body)))
	    (cons 'define (cons decl body)))))  ; (define (f ...) ...)
  (define-1 name (apply values body)))          ; there are examples in s7test.scm
