...
(define lint
  ...
  (let ((no-side-effect-functions
	 (let ((ht (make-hash-table)))
	   (for-each
	    ...
	    '(* + - / < <= = > >=
	      ...
	      funclet? bignum? weak-hash-table? goto? port-file port-string byte? hash-code goto-active? type-name
	...
	(built-in-functions (let ((ht (make-hash-table)))
				 ...
				 (set! (ht op) #t))
			       '(symbol? gensym? keyword? let? openlet? iterator? macro? c-pointer? c-object? c-object-type c-object-let constant? subvector?
			         ...
			         newline write display read-char peek-char write-char write-string read-byte write-byte read-bytes write-bytes
				 ...
				 complex-vector make-complex-vector complex-vector-ref complex-vector-set! goto-active? type-name
	...
	(side-effect-functions '(provide list-set! fill! reverse! sort! vector-fill! vector-set! float-vector-set! int-vector-set! byte-vector-set! complex-vector-set!
				 ...
				 get-output-string newline read-char peek-char write-char write-string read-byte write-byte read-bytes write-bytes read-line read-string
	...
	(bools (let ((h (make-hash-table)))
		 (for-each
		  ...
		  '(symbol? byte? integer? rational? real? number? complex? float? keyword? gensym? byte-vector? string? list? sequence?
		    char? boolean? float-vector? int-vector? vector? let? hash-table? input-port? null? pair? proper-list? goto-active?
	...
	(booleans (let ((h (make-hash-table)))
		  (for-each
		   ...
		   '(symbol? byte? integer? rational? real? number? complex? float? keyword? gensym? byte-vector? string? list? sequence?
		     ...
		     negative? positive? zero? syntax? undefined? tree-cyclic? not openlet? ; immutable? constant? goto-active?
	...
	(syntaces (let ((h (make-hash-table)))
		    (for-each
		     ...
		     '(quote #_quote if begin let let* letrec letrec* cond case or and do set! unless when
		       ...
		       lambda lambda* define define* macro macro* bacro bacro*
		       define-macro define-macro* define-bacro define-bacro*
		       define-constant define-expansion define-expansion* expansion expansion*))
		    h))

	(definers '(define define* define-constant lambda lambda* curlet require load eval eval-string
		    define-macro define-macro* define-bacro define-bacro* define-expansion define-expansion*
...
|#
			    ...
			    (cond ((not (pair? endb))
				  ...
				  ((side-effect? endb env)
				   (list (case (car endb)
					   ...
					   ((write-char write-byte write-bytes string-copy openlet float-vector-set! read-char read-byte read-bytes
	...
	(lambda (f env)
	  (or (hash-table-ref macros f)
		...
		(and fd
		     (memq (var-ftype fd) '(define-macro define-macro* define-expansion define-expansion*
    ...
    (define (define->type c)
      (and (pair? c)
	   (case (car c)
	     ...
	     ((dilambda) 'dilambda?)
	     ((define-macro define-macro* define-bacro define-bacro* define-expansion define-expansion*) 'macro?)
    ...
    (define (side-effect-with-vars? form env vars)
	  ...
	  (or (and (not (hash-table-ref no-side-effect-functions (car form))) ; includes quote, let, etc
		...
		((define-constant define-expansion define-expansion*) #t)
	...
	(hash-special '*s7*
	 (let ((s7-fields (let ((h (make-hash-table)))
			    (for-each (lambda (f)
					...
					number-separator symbol-quote? symbol-printer make-function scheme-version))
			    h)))
	   (lambda (caller head form env)
	     (if (len=2? form)
		 (let ((arg (cadr form)))
			   ...
			   (let fwalk ((sym par) (tree vhead))
			     (when (pair? tree)
				   ...
				   (case (car tree)
				     ...
				     ((define* define-macro define-macro* define-expansion define-expansion* define-bacro define-bacro*)
				      (if (and (len>1? (cdr tree))
					       (pair? (cadr tree))
					       (not (lint-any? (lambda (a) (eq? sym a)) (args->proper-list (cdadr tree)))))
					  (fwalk sym (cddr tree))))
...
|#
	...
	(define (redundant-return caller body prev-f f env)
	  (case (car prev-f)
	    ...
	    ((set! define define* define-macro define-constant define-macro* define-expansion define-expansion* define-bacro define-bacro*)
...
|#
		    ...
		    (when (and (= ctr (- len 1))
			       (len>1? f))
		      (if (and (pair? (cadr f))
			       (memq (car f) '(define define* define-macro define-constant define-macro* define-expansion define-expansion* define-bacro define-bacro*)))
    ...
    (define walker-functions
      (let ((binders '(let let* letrec letrec* do
		       ...
		       define-macro define-macro* define-bacro define-bacro* define-constant define-expansion define-expansion*
		 ...
		 (lambda (quit)
		   (let walker ((code (cdddr form))) ; get anything that changes in the loop
		     (when (pair? code)
			   ...
			   (cond ((and (memq (car code) '(define* define-macro define-macro* define-bacro define-bacro* define-expansion define-expansion*))
...
|#
	...
	(let ()
	  ...
	  (define let-walker
	    (let ((input-side-effect-functions '(;set-current-input-port close-input-port open-input-file open-input-string
						 read-char peek-char read-byte read-bytes read-line read-string read))
		  (output-side-effect-functions '(;set-current-output-port close-output-port flush-output-port open-output-file open-output-string
						  ...
						  newline write-char write-string display write write-byte write-bytes format)))
			  ...
			  (when (and (not named-let)
			    ...
			    (when (and (> (length body) 3)
				       ...
				       (not (tree-set-car-member '(define define* define-macro define-macro*
								    define-bacro define-bacro* define-constant define-expansion define-expansion*)
								 body)))
	...
	(lambda (new-form leaves env fvar orig-form)
	  (unless (lint-tree-set-memq '(define define*
				    ...
				    define-constant define-macro define-macro* define-expansion define-expansion*
