  ...
  (set! *mock-vector*
	(let* ((mock-vector? #f)
		...
		(inlet 'local-set!         (lambda (obj i val)          ; reactive-vector uses this as a hook into vector-set!
		       ...
		       'complex-vector?    (with-mock-wrapper #_complex-vector?)
		...
		(inlet 
		 ...
		 '>=               (with-mock-wrapper* #_>=)
 		 ...
 		 'make-polar       (if (provided? 'pure-s7)
 				       ...
 				       (lambda (mag ang) (#_make-polar (->value mag) (->value ang))))
		 ...
		 'make-complex-vector (with-mock-wrapper* #_make-complex-vector)
		 ...
		 'vector-ref          (with-mock-wrapper* #_vector-ref)
		 'complex-vector-ref  (with-mock-wrapper* #_complex-vector-ref)
		 ...
		 'complex-vector-set! (with-mock-wrapper* #_complex-vector-set!)
  ...
  (set! *mock-port*
	(let* ((mock-port? #f)
		...
		(inlet 'input-port?         (with-mock-wrapper #_input-port?)
		       ...
		       'read-bytes          (with-mock-wrapper* #_read-bytes)
		       ...
		       'write-string        (with-mock-wrapper* #_write-string)
		       'write-byte          (with-mock-wrapper* #_write-byte)
		       'write-bytes         (with-mock-wrapper* #_write-bytes)
		       'read-string         (with-mock-wrapper* #_read-string)
