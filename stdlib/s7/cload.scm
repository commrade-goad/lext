...
(define* (c-define-1 cur-env function-info (prefix "") (headers ()) (cflags "") (ldflags "") output-name)
  ...
  (let ((file-name (string-append *cload-directory* 
	...
	(let ((all-names (hash-table)))
	  (lambda (name)
	    (if (not (eq? (hash-table-ref all-names name) (*s7* 'hash-table-missing-key-value)))
	...
	(let ()
      ...
      (unless (and output-name
		   ...
		   (provided? 'system-extras) ; too late!
	...
	(delete-file o-file-name))
      ...
      (if (defined? 'init_func cur-env #t)
	  (let-set! cur-env 'init_func (string->symbol init-name))
