...
(define-macro (internal-basic-c-import scheme-name lib-handle c-name ret-type arg-types . nfixed)
...
(internal-basic-c-import internal-basic-c-memcpy-raw internal-basic-libc "memcpy" pointer (pointer pointer ulong))
(internal-basic-c-import internal-basic-c-memset-raw internal-basic-libc "memset" pointer (pointer int ulong))
(internal-basic-c-import internal-basic-malloc       internal-basic-libc "malloc" pointer (ulong))
(internal-basic-c-import internal-basic-free         internal-basic-libc "free"   void    (pointer))
...
(define-macro (with-heap-alloc var-type . body)
  (let* ((var (car var-type))
         ...
         (ptr (gensym)))
    `(let ((,ptr (internal-basic-malloc ,size)))
       (dynamic-wind
         (lambda () #f)
         (lambda ()
...
(define-macro (with-c-string var-str . body)
  (let ((var (car var-str))
    ...
    `(let* ((,str ,str-expr)
            ...
            (,ptr (internal-basic-malloc (+ ,len 1))))
       (let loop ((i 0))
         (if (< i ,len)
             (begin
...
(define-macro (with-c-array var-type-list . body)
  (let ((var (car var-type-list))
    ...
    `(let* ((,lst ,lst-expr)
            (,len (length ,lst))
            (,size (c-size ',type))
            (,ptr (internal-basic-malloc (* ,len ,size))))
...
(define-macro (with-c-string-array var-lst . body)
  (let ((var (car var-lst))
    ...
    `(let* ((,lst ,lst-expr)
            ...
            (,ptr (internal-basic-malloc (* (+ ,len 1) ,psize)))
            (,ptrs '()))
       (let loop ((curr ,lst) (i 0))
         (if (not (null? curr))
             (let* ((str (car curr))
                    (slen (string-length str))
                    (sptr (internal-basic-malloc (+ slen 1))))
               (let str-loop ((j 0))
                 (if (< j slen)
                     (begin
       ...
       (dynamic-wind
         ...
         (lambda ()
           (let ((,var (internal-basic-tptr 'pointer ,ptr)))
             ,@body))
         (lambda ()
           (for-each internal-basic-free ,ptrs)
           (internal-basic-free ,ptr))))))

;; --- Capturing Engine ---
(internal-basic-c-import internal-basic-c-fflush  internal-basic-libc "fflush"  int     (pointer))
(internal-basic-c-import internal-basic-c-dup     internal-basic-libc "dup"     int     (int))
(internal-basic-c-import internal-basic-c-dup2    internal-basic-libc "dup2"    int     (int int))
(internal-basic-c-import internal-basic-c-close   internal-basic-libc "close"   int     (int))
(internal-basic-c-import internal-basic-c-tmpfile internal-basic-libc "tmpfile" pointer ())
(internal-basic-c-import internal-basic-c-fileno  internal-basic-libc "fileno"  int     (pointer))
(internal-basic-c-import internal-basic-c-rewind  internal-basic-libc "rewind"  void    (pointer))
(internal-basic-c-import internal-basic-c-fclose  internal-basic-libc "fclose"  int     (pointer))
(internal-basic-c-import internal-basic-c-fgetc   internal-basic-libc "fgetc"   int     (pointer))
