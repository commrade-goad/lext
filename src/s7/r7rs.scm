...
(define* (write-bytevector bv (port (current-output-port)) (start 0) end)
  (write-bytes port (subvector bv start (or end (length bv))))
  bv)

(define* (read-bytevector! bv (port (current-input-port)) (start 0) end)
  (let ((v (subvector bv start (or end (length bv)))))
    (let ((nv (read-bytes port (length v) v)))
      (if (zero? (vector-length nv)) #<eof> nv)))) ; why does r7rs insist on #<eof> here? #u() is better

(define* (read-bytevector k port)
  (let ((bv (read-bytes port k)))
    (if (zero? (vector-length bv)) #<eof> bv)))
