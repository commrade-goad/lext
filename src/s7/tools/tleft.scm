...
(f71 tc-size) ; op_safe_closure_3a[checked] if_a_a_p[fx_lt_si] if_b_a_p[fb_num_eq_s0]
...
(define-constant (rc3 lst) ; does this pattern ever happen? maybe opA_LAq is more likely
  (or (null? lst)
      (and (symbol? (car lst))
	   (null? (rc3 (cdr lst))))))
...
(define (tcase1 n)       ; opt_case, numbers_are_eqv
  (do ((i 0 (+ i 1)))
      ((= i n))
    (case i
      ((0) 0)
...
(tcase3 '(1 2) case-size)

(define (tcase3-1 lst n)
