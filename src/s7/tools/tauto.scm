...
(define baddies '(exit emergency-exit abort autotest s7-optimize dynamic-unwind
		  ...
		  make-rectangular macro macro* bacro bacro* expansion expansion*
...
(let ((probes (vector #f #t () #\a #<undefined> #<unspecified> #<eof>
		      ...
		      (make-iterator '(1 2 3))))
      (inputters '(read read-string read-line read-byte read-bytes read-char))
      (outputters '(write display write-string write-char write-byte write-bytes newline))
