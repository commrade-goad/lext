...
(define (complex-fft data n dir)
  (when data
	...
	((= i n))
      (if (> j i)
	  (let ((temp (complex-vector-ref data j)))
	    (complex-vector-set! data j (complex-vector-ref data i))
	    (complex-vector-set! data i temp)))
      (let ((m (/ n 2)))
	(do () 
	    ((not (<= 2 m j)))
	  (set! j (- j m))
	...
	(let ((wpc (exp theta))
	  ...
	  (do ((ii 0 (+ ii 1)))
	    ...
	    (do ((jj 0 (+ jj 1))
		 ...
		 (j (+ ii prev) (+ j mmax)))
		((>= jj pow))
	      (set! tc (* wc (complex-vector-ref data j)))
	      (complex-vector-set! data j (- (complex-vector-ref data i) tc))
	      (complex-vector-set! data i (+ (complex-vector-ref data i) tc)))
	    (set! wc (* wc wpc)))
	  (set! prev mmax))))
    data))

(define (complex-checker fvr n)
  (when fvr
    (let ((pk 0.0)
	  (mx 0.0)
	  (mxloc 0))
      (do ((i 0 (+ i 1)))
	  ((= i n))
	(set! pk (magnitude (complex-vector-ref fvr i)))
	(when (> pk mx)
	  (set! mx pk)
	  (set! mxloc i)))
      (format () "~A ~A~%" mxloc (/ (* 2 (sqrt mx)) n))
      (format () "~A ~A~%" (magnitude (fvr 4)) (magnitude (fvr (- n 4)))))
    (complex-vector-set! fvr 4 0.0)
    (complex-vector-set! fvr (- n 4) 0.0)
    (let ((mx 0.0))
      (do ((i 0 (+ i 1)))
	  ((= i n))
	(set! mx (max mx (magnitude (complex-vector-ref fvr i)))))
      (format () "noise: ~A~%" mx))))

(define (complex-test n)
  (let ((fvr (make-vector n 0.0 complex?))
	(scl (/ (* 8.0 pi) n)))
    (do ((i 0 (+ i 1))
	 (x 0.0 (+ x scl)))
	((= i n))
      (complex-vector-set! fvr i (sin x)))
    (complex-fft fvr n 1)
    (complex-checker fvr n)
    (complex-fft #f 0 0)
    (complex-checker #f n)))
(complex-test fft-size)
(clear-and-gc)
...
(define (ratio-checker fvr fvi n)
  (when fvr
    (let ((vr 0.0)
	  (vi 0.0)
	  (pk 0.0)
	...
	(when (> pk mx)
	  ...
	  (set! mxloc i)))
      (format () "~A ~A~%" mxloc (/ (* 2 (sqrt mx)) n))
      (format () "~A ~A~%" (* 1.0 (fvi 4)) (* 1.0 (fvi (- n 4)))))
    (set! (fvr 4) 0)
    (set! (fvi 4) 0)
    (set! (fvr (- n 4)) 0)
    (set! (fvi (- n 4)) 0)
...
(define (ratio-test n)
  (let ((fvr (make-vector n 0 rational?))
	(fvi (make-vector n 0 rational?))
	(scl (/ (* 8.0 pi) n)))
    (do ((i 0 (+ i 1))
	 (x 0.0 (+ x scl)))
	((= i n))
      (vector-set! fvr i (rationalize (sin x))))
    (ratio-fft fvr fvi n 1)
    (ratio-checker fvr fvi n)
    (ratio-fft #f #f n 1)
    (ratio-checker #f #f n)))
...
(when (> total-memory (* 18 big-size))
  ...
  (let ((bigfv (make-float-vector big-size 0.5)))
    ...
    (define (big-float-vector-filler lim)
      (do ((i 0 (+ i 1)))
	  ((= i lim))
	(float-vector-set! bigfv i 1.0)))

    (big-float-vector-filler big-size)
    (test (bigfv 1) 1.0)))
...
(define (float-vector-checker fvr fvi n)
  (when fvr
    (let ((vr 0.0)
	  (vi 0.0)
	  (pk 0.0)
	...
	(when (> pk mx)
	  ...
	  (set! mxloc i)))
      (format () "~A ~A~%" mxloc (/ (* 2 (sqrt mx)) n))
      (format () "~A ~A~%" (fvi 4) (fvi (- n 4))))
    (float-vector-set! fvr 4 0.0)
    (float-vector-set! fvi 4 0.0)
    (float-vector-set! fvr (- n 4) 0.0)
    (float-vector-set! fvi (- n 4) 0.0)
    (let ((mx 0.0))
      (do ((i 0 (+ i 1)))
	  ((= i n))
...
(define (float-vector-test n)
  (let ((fvr (make-float-vector n))
	(fvi (make-float-vector n 0.0))
	(scl (/ (* 8.0 pi) n)))
    (do ((i 0 (+ i 1))
	 (x 0.0 (+ x scl)))
	((= i n))
      (float-vector-set! fvr i (sin x)))
    (float-vector-fft fvr fvi n 1)
    (float-vector-checker fvr fvi n)
    (float-vector-fft #f #f n 1)
    (float-vector-checker #f #f n)))

(float-vector-test fft-size)
(clear-and-gc)
...
(define (float-2d-checker fvr n)
  (when fvr
	...
	(when (> pk mx)
	  ...
	  (set! mxloc i)))
      (format () "~A ~A~%" mxloc (/ (* 2 (sqrt mx)) n))
      (format () "~A ~A~%" (fvr 1 4) (fvr 1 (- n 4))))
    (float-vector-set! fvr 0 4 0.0)
    (float-vector-set! fvr 1 4 0.0)
    (float-vector-set! fvr 0 (- n 4) 0.0)
    (float-vector-set! fvr 1 (- n 4) 0.0)
    (let ((mx 0.0))
      (do ((i 0 (+ i 1)))
	  ((= i n))
...
(define (float-2d-test skip n)
  (format *stderr* "float-2d ~A~%" skip)
  (let ((fvr (make-float-vector (list skip n) 0.0))
	(scl (/ (* 8.0 pi) n)))
    (do ((i 0 (+ i 1))
	 (x 0.0 (+ x scl)))
	((= i n))
      (float-vector-set! fvr 0 i (sin x)))
    (float-2d-fft fvr n 1)
    (float-2d-checker fvr n)
    (float-2d-fft #f n 1)
    (float-2d-checker #f n)))

(float-2d-test 2 fft-size)
(clear-and-gc)
(float-2d-test 2 32)

;; now try a big float-vector, but same size fft
(when (> total-memory (* 9 (ash 1 31)))
  (float-2d-test (ash 1 14) fft-size)) ; fft-size (ash 1 17) -> (* 8 (ash 1 31))
(float-2d-test 2 32)
(clear-and-gc)
  ...
  (let ((bigfv (make-int-vector big-size 5)))
	...
	(let ((big2 (subvector big1 (- (ash 1 31) 10) (+ (ash 1 31) 10))))
	  ...
	  (let ((big3 (make-int-vector 20 0)))
	    (copy bigfv big3 (- (ash 1 31) 10) (+ (ash 1 31) 10))
	    (test (equivalent? big2 big3) #t)))))
    (define (big-int-vector-filler lim)
      (do ((i 0 (+ i 1)))
	  ((= i lim))
	(int-vector-set! bigfv i 1)))
    (big-int-vector-filler big-size)
    (test (bigfv 1) 1)))
...
(define (int-vector-checker fvr fvi n)
  (when fvr
    (let ((vr 0)
	  (vi 0)
	  (pk 0)
    ...
    (int-vector-set! fvr (- n 4) 0)
    (int-vector-set! fvi (- n 4) 0)
    (let ((mx 0.0))
      (do ((i 0 (+ i 1)))
	  ((= i n))
...
(define (int-vector-test n)
  (let ((fvr (make-int-vector n))
	(fvi (make-int-vector n 0))
	(scl (/ (* 8.0 pi) n)))
    (do ((i 0 (+ i 1))
	 (x 0.0 (+ x scl)))
	((= i n))
      (int-vector-set! fvr i (round (* 1000.0 (sin x)))))
    (int-vector-fft fvr fvi n 1)
    (int-vector-checker fvr fvi n)
    (int-vector-fft #f #f n 1)
    (int-vector-checker #f #f n)))

(int-vector-test fft-size)
(clear-and-gc)
...
(define (vector-checker fvr fvi n)
  (when fvr
    (let ((vr 0.0)
	  (vi 0.0)
	  (pk 0.0)
    ...
    (vector-set! fvr (- n 4) 0.0)
    (vector-set! fvi (- n 4) 0.0)
    (let ((mx 0.0))
      (do ((i 0 (+ i 1)))
	  ((= i n))
	(set! mx (max mx (abs (vector-ref fvr i)) (abs (vector-ref fvi i)))))
...
(define (vector-test n)
  (let ((fvr (make-vector n 0.0 real?))
	(fvi (make-vector n 0.0 real?))
	(scl (/ (* 8.0 pi) n)))
    (do ((i 0 (+ i 1))
	 (x 0.0 (+ x scl)))
	((= i n))
      (vector-set! fvr i (sin x)))
    (vector-fft fvr fvi n 1)
    (vector-checker fvr fvi n)
    (vector-fft #f #f n 1)
    (vector-checker #f #f n)))

(vector-test fft-size)
(clear-and-gc)
...
(define (vector-2d-checker fvr n)
  (when fvr
    (let ((vr 0.0)
	  (vi 0.0)
	  (pk 0.0)
    ...
    (vector-set! fvr 0 (- n 4) 0.0)
    (vector-set! fvr 1 (- n 4) 0.0)
    (let ((mx 0.0))
      (do ((i 0 (+ i 1)))
	  ((= i n))
...
(define (vector-2d-test skip n)
  (let ((fvr (make-vector (list skip n) 0.0 real?))
	(scl (/ (* 8.0 pi) n)))
    (do ((i 0 (+ i 1))
	 (x 0.0 (+ x scl)))
	((= i n))
      (vector-set! fvr 0 i (sin x)))
    (vector-2d-fft fvr n 1)
    (vector-2d-checker fvr n)
    (vector-2d-fft #f n 1)
    (vector-2d-checker #f n)))

(vector-2d-test 2 fft-size)
(clear-and-gc)
...
(define (hash-test n)
  (let ((fvr (make-hash-table n)) ; (make-hash-table n #t (cons #t float?))
	(fvi (make-hash-table n))
	(scl (/ (* 8.0 pi) n))
	(vr 0+i)
	(vi 0+i)
	(pk 0+i))
    ...
    (hash-table-set! fvr (- n 4) 0.0)
    (hash-table-set! fvi (- n 4) 0.0)
    (do ((mx 0.0)
	 (i 0 (+ i 1)))
	((= i n)
	 (format () "noise: ~A~%" mx))
      (set! mx (max mx (abs (hash-table-ref fvr i)) (abs (hash-table-ref fvi i)))))
    (hash-table-fft #f #f n 1)))
(hash-test fft-size)
(clear-and-gc)
...
(define (hash-symbol-test n)
  (let ((fvr (hash-table))
	(fvi (hash-table))
	(rl-syms (make-vector n))
	(im-syms (make-vector n))
	(scl (/ (* 8.0 pi) n))
	(vr 0+i)
	(vi 0+i)
	(pk 0+i))
    ...
    (hash-symbol-fft fvr fvi n rl-syms im-syms 1)
    (do ((mx 0.0)
	 (mxloc 0)
	 (i 0 (+ i 1)))
	((= i n) 
    ...
    (hash-table-set! fvr (vector-ref rl-syms (- n 4)) 0.0)
    (hash-table-set! fvi (vector-ref im-syms (- n 4)) 0.0)
    (do ((mx 0.0)
	 (i 0 (+ i 1)))
	((= i n)
	 (format () "noise: ~A~%" mx))
    ...
    (fill! fvr #f)
    (fill! fvi #f)
    (hash-symbol-fft #f #f n #f #f 1)))
(hash-symbol-test fft-size)
(clear-and-gc)
...
(define (let-test n)
  (let ((fvr (inlet))
    ...
    (let-fft fvr fvi n rl-syms im-syms 1)
    (do ((mx 0.0)
	 (mxloc 0)
	 (i 0 (+ i 1)))
	((= i n) 
    ...
    (let-set! fvr (vector-ref rl-syms (- n 4)) 0.0)
    (let-set! fvi (vector-ref im-syms (- n 4)) 0.0)
    (do ((mx 0.0)
	 (i 0 (+ i 1)))
	((= i n)
	 (format () "noise: ~A~%" mx))
...
(define (string-checker fvr fvi n)
  (when fvr
    (let ((vr 0.0+i)
	  (vi 0.0+i)
	  (pk 0.0+i)
	...
	(when (> pk mx)
	  ...
	  (set! mxloc i)))
      (format () "~A ~A~%" mxloc (/ (* 2 (sqrt mx)) n))
      (format () "~A ~A~%" (string->number (vector-ref fvi 4)) (string->number (vector-ref fvi (- n 4)))))
    (vector-set! fvr 4 "0.0")
    (vector-set! fvi 4 "0.0")
    (vector-set! fvr (- n 4) "0.0")
    (vector-set! fvi (- n 4) "0.0")
...
(define (string-test n)
  (let ((fvr (make-vector n))
	(fvi (make-vector n "0.0"))
	(scl (/ (* 8.0 pi) n)))
    (do ((i 0 (+ i 1))
	 (x 0.0 (+ x scl)))
	((= i n))
      (vector-set! fvr i (number->string (sin x))))
    (string-fft fvr fvi n 1)
    (string-checker fvr fvi n)
    (string-fft #f #f n 1)
    (string-checker #f #f n)
    (fill! fvr #f)
    (fill! fvi #f)))
(string-test fft-size)
...
(define (string-radix-checker fvr fvi n)
  (when fvr
    (let ((vr 0.0)
	  (vi 0.0)
	  (pk 0.0)
...
(define (string-radix-test n)
  (let ((fvr (make-vector n))
    ...
    (string-radix-fft fvr fvi n 1)
    (string-radix-checker fvr fvi n)
    (fill! fvr #f)
    (fill! fvi #f)
    (string-radix-fft #f #f n 1)
    (string-radix-checker #f #f n)))
(string-radix-test fft-size))
...
(define (port-test n)
  (let ((fvr (make-vector n))
	...
	((= i n))
      (vector-set! fvr i (number->string (sin x))))
    (port-fft fvr fvi n 1)
    (port-checker fvr fvi n)
    (port-fft #f #f n 1)
    (port-checker #f #f n)
    (fill! fvr #f)
    (fill! fvi #f)))
...
(define (complex-2d-checker fvr n)
  (when fvr
    (let ((pk 0.0)
	  (mx 0.0)
	  (mxloc 0))
	...
	(when (> pk mx)
	  ...
	  (set! mxloc i)))
      (format () "~A ~A~%" mxloc (/ (* 2 (sqrt mx)) n))
      (format () "~A ~A~%" (magnitude (old-value (fvr 4))) (magnitude (old-value (fvr (- n 4))))))
    (vector-set! fvr 4 (new-value 0.0))
    (vector-set! fvr (- n 4) (new-value 0.0))
    (let ((mx 0.0))
      (do ((i 0 (+ i 1)))
	  ((= i n))
...
(define (complex-2d-test n)
  (let ((fvr (make-vector n))
    ...
    (complex-2d-fft fvr n 1)
    (complex-2d-checker fvr n)
    (complex-2d-fft #f n 1)
    (complex-2d-checker #f n)
    (fill! fvr #f)))
(complex-2d-test fft-size)
...
(define (complex-hash-checker fvr n)
  (when fvr
    (let ((pk 0.0)
	  (mx 0.0)
	  (mxloc 0))
	...
	(when (> pk mx)
      ...
      (format () "~A ~A~%" 
	      ...
	      (magnitude (hash-table-ref (vector-ref fvr (- n 4)) 'z))))
    (vector-set! fvr 4 (hash-table 'z 0.0))
    (vector-set! fvr (- n 4) (hash-table 'z 0.0))
    (let ((mx 0.0))
      (do ((i 0 (+ i 1)))
	  ((= i n))
...
(define (complex-hash-test n)
  (let ((fvr (make-vector n))
    ...
    (complex-hash-fft fvr n 1)
    (complex-hash-checker fvr n)
    (complex-hash-fft #f n 1)
    (complex-hash-checker #f n)
    (fill! fvr #f)))
(complex-hash-test fft-size)
...
(define (iterator-vector-checker fvr fvi n)
  (when fvr
    (let ((vr 0.0+i)
	  (vi 0.0+i)
	  (pk 0.0+i)
	...
	(when (> pk mx)
	  ...
	  (set! mxloc i)))
      (format () "~A ~A~%" mxloc (/ (* 2 (sqrt mx)) n))
      (format () "~A ~A~%" (iterate (vector-ref fvi 4)) (iterate (vector-ref fvi (- n 4)))))
    (vector-set! fvr 4 (make-iterator (make-float-vector 2 0.0)))
    (vector-set! fvi 4 (make-iterator (make-float-vector 2 0.0)))
    (vector-set! fvr (- n 4) (make-iterator (make-float-vector 2 0.0)))
    (vector-set! fvi (- n 4) (make-iterator (make-float-vector 2 0.0)))
    (let ((mx 0.0))
      (do ((i 0 (+ i 1)))
	  ((= i n))
...
(define (iterator-vector-test n)
  (let ((fvr (make-vector n))
    ...
    (iterator-vector-fft fvr fvi n 1)
    (iterator-vector-checker fvr fvi n)
    (iterator-vector-fft #f #f n 1)
    (iterator-vector-checker #f #f n)
    (fill! fvr #f)
    (fill! fvi #f)))

(iterator-vector-test fft-size)
(clear-and-gc)
...
(define (complex-closure-checker fvr n)
  (when fvr
    (let ((pk 0.0)
	  (mx 0.0)
	  (mxloc 0))
	...
	(when (> pk mx)
      ...
      (format () "~A ~A~%" 
	      ...
	      (magnitude ((vector-ref fvr (- n 4))))))
    (vector-set! fvr 4 (let ((z 0.0)) (lambda () z)))
    (vector-set! fvr (- n 4) (let ((z 0.0)) (lambda () z)))
    (let ((mx 0.0))
      (do ((i 0 (+ i 1)))
	  ((= i n))
...
(define (complex-closure-test n)
  (let ((fvr (make-vector n))
    ...
    (complex-closure-fft fvr n 1)
    (complex-closure-checker fvr n)
    (complex-closure-fft #f n 1)
    (complex-closure-checker #f n)
    (fill! fvr #f)))
(complex-closure-test fft-size)
...
(define (block-vector-checker fvr fvi n)
  (when fvr
    (let ((vr 0.0)
	  (vi 0.0)
	  (pk 0.0)
    ...
    (vector-set! fvr (- n 4) (block 0.0))
    (vector-set! fvi (- n 4) (block 0.0))
    (let ((mx 0.0))
      (do ((i 0 (+ i 1)))
	  ((= i n))
...
(define (block-vector-test n)
  (let ((fvr (make-vector n))
	(fvi (make-vector n (block 0.0)))
	(scl (/ (* 8.0 pi) n)))
    (do ((i 0 (+ i 1))
	 (x 0.0 (+ x scl)))
	((= i n))
      (vector-set! fvr i (block (sin x))))
    (block-vector-fft fvr fvi n 1)
    (block-vector-checker fvr fvi n)
    (block-vector-fft #f #f n 1)
    (block-vector-checker #f #f n)))

(block-vector-test fft-size)
;(clear-and-gc)
