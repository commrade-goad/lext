...
static char *dax_name = NULL;
static char *make_dax_name(void) {dax_name = strdup("make-dax"); return(dax_name);}
static void free_dax_name(void)  {free(dax_name); dax_name = NULL;}
...
static s7_pointer g_incr_x(s7_scheme *sc, s7_pointer args)
{
  s7_pointer let = s7_car(args), incr = s7_cadr(args);
  s7_pointer old_curlet = s7_set_curlet(sc, let);
  s7_pointer x = s7_slot(sc, s7_make_symbol(sc, "x"));
  s7_set_curlet(sc, old_curlet);
  s7_slot_set_value(sc, x, s7_make_integer(sc, s7_integer(s7_slot_value(x)) + s7_integer(incr)));
  return(s7_slot_value(x));
}
...
int main(int argc, char **argv)
{
...

  {
    ...
    if (strcmp(b1, S7_VERSION) != 0) fprintf(stderr, "version mismatch in s7.h: %d.%d != %s\n", S7_MAJOR_VERSION, S7_MINOR_VERSION, S7_VERSION);
  ...
  if (strcmp(s7_string(s7_type_name(sc, p)), "a vector") != 0)
    fprintf(stderr, "%d: type-name(vector) %s\n", __LINE__, s7_string(s7_type_name(sc, p)));
...

  {
    ...
    p = s7_eval(sc, c1234, s7_sublet(sc, s7_rootlet(sc), s7_rootlet(sc)));
    if (s7_integer(p) != 9)
    ...
    p = s7_eval_with_location(sc, s7_cons(sc, s7_make_symbol(sc, "+"),
					  s7_cons(sc, s7_make_integer(sc, 1),
						  s7_cons(sc, s7_make_integer(sc, 2), s7_nil(sc)))),
			      s7_rootlet(sc), "ffitest", __FILE__, __LINE__);
    if (s7_integer(p) != 3)
       {fprintf(stderr, "%d: (eval '(+ 1 2)) is %s?\n", __LINE__, s1 = TO_STR(p)); free(s1);}
    p = s7_eval(sc, s7_cons(sc, s7_make_symbol(sc, "+"), /* s7-ffi.html example */
               s7_cons(sc, s7_make_integer(sc, 1),
  ...
  s7_define_function(sc, make_dax_name(), make_dax, 2, 0, false, "(make-dax x data) makes a new dax");
  free_dax_name(); /* a test that the name is copied */
  s7_define_typed_function(sc, "dax?", is_dax, 1, 0, false, "(dax? anything) returns #t if its argument is a dax object", 
			   s7_make_signature(sc, 2, s7_make_symbol(sc, "boolean?"), s7_t(sc)));
  if (s7_cadr(s7_signature(sc, s7_name_to_value(sc, "dax?"))) != s7_t(sc))
  ...
  s7_define_variable(sc, "dax-xx",
                     s7_typed_dilambda(sc, "dax-x", dax_x, 1, 0, set_dax_x, 2, 0, "dax x field (a real)",
				       s7_make_signature(sc, 2, s7_make_symbol(sc, "real?"), s7_make_symbol(sc, "c-object?")),
				       s7_make_signature(sc, 3, s7_make_symbol(sc, "real?"), s7_make_symbol(sc, "c-object?"), s7_make_symbol(sc, "real?"))));
  ...
  p = s7_eval_c_string(sc, "(make-dax 1.0 (list 2))");
  gc_loc = s7_gc_protect(sc, p);
  if (!s7_is_c_object(p))
    {fprintf(stderr, "%d: %s is not a c_object?\n", __LINE__, s1 = TO_STR(p)); free(s1);}
  p1 = s7_apply_function(sc, s7_name_to_value(sc, "dax?"), s7_cons(sc, p, s7_nil(sc)));
  ...
  {
    s7_pointer let = s7_inlet(sc, s7_list(sc, 1, s7_cons(sc, s7_make_symbol(sc, "x"), s7_make_integer(sc, 123))));
    s7_pointer sig = s7_make_signature(sc, 3, s7_make_symbol(sc, "integer?"), s7_make_symbol(sc, "let?"), s7_make_symbol(sc, "integer?"));
    s7_pointer incr_x = s7_defun(sc, "incr-x", g_incr_x, 2, 0, false, "incr-x doc", sig, let, s7_safe);
    s7_int gc1 = s7_gc_protect(sc, let); /* sig and incr_x are already GC protected */
    p = g_incr_x(sc, s7_cons(sc, let, s7_cons(sc, s7_make_integer(sc, 32), s7_nil(sc))));
    if (s7_integer(p) != 123 + 32)
      {fprintf(stderr, "%d: x is %s\n", __LINE__, s1 = TO_STR(p)); free(s1);}
    p = s7_eval_c_string_with_environment(sc, "(incr-x (curlet) 32)", let);
    if (s7_integer(p) != 123 + 32 + 32)
      {fprintf(stderr, "%d: x is %s\n", __LINE__, s1 = TO_STR(p)); free(s1);}
    p = s7_let_ref(sc, let, s7_make_symbol(sc, "incr-x"));
    if (p != incr_x)
      {fprintf(stderr, "%d: incr-x is %s\n", __LINE__, s1 = TO_STR(p)); free(s1);}
    if (strcmp(s7_documentation(sc, incr_x), "incr-x doc") != 0)
      fprintf(stderr, "%d: doc: %s\n", __LINE__, s7_documentation(sc, incr_x));
    p = s7_signature(sc, incr_x);
    if ((!s7_is_pair(p)) || (s7_list_length(sc, p) != 3))
      {fprintf(stderr, "%d: incr-x signature is %s\n", __LINE__, s1 = TO_STR(p)); free(s1);}
    s7_gc_unprotect_at(sc, gc1);
  }
...

  {
    ...
    s7_define_typed_function_star(sc, "fs2", fs2, "", NULL, s7_make_signature(sc, 2, s7_t(sc), s7_t(sc)));
  ...
  p = s7_eval_c_string(sc, "(list (values4))");
  if (!s7_is_pair(p))
    {fprintf(stderr, "%d: s7_values4 not a pair? %s\n", __LINE__, s1 = TO_STR(p)); free(s1);}
  if (s7_integer(s7_list_ref(sc, p, 2)) != 2)
    {fprintf(stderr, "%d: s7_values4[2]=%s\n", __LINE__, s1 = TO_STR(p)); free(s1);}
...

  {
    ...
    if (s7_is_integer(s7_symbol_local_value(sc, s7_make_symbol(sc, "new-env-var"), s7_rootlet(sc))))
      {fprintf(stderr, "%d: new-env-var is globally defined?\n", __LINE__);}
    s7_set_curlet(sc, old_env);
    if (s7_is_defined(sc, "new-env-var"))
      {fprintf(stderr, "%d: new-env-var is defined globally?\n", __LINE__);}
    s7_set_curlet(sc, new_env);
    p = s7_symbol_local_value(sc, s7_make_symbol(sc, "new-env-var"), new_env);
    if (s7_integer(p) != 123)
      {fprintf(stderr, "%d: new-env-var: %s\n", __LINE__, s1 = TO_STR(p)); free(s1);}
    p = s7_slot(sc, s7_make_symbol(sc, "new-env-var"));
    if (!s7_is_immutable(p))
      fprintf(stderr, "%d: new-env-var in new_env is not immutable\n", __LINE__);

    {
      s7_pointer another_let = s7_sublet(sc, s7_rootlet(sc), s7_nil(sc));
      s7_int another_loc = s7_gc_protect(sc, another_let);
      s7_define(sc, another_let, s7_make_symbol(sc, "new-env-var"), s7_make_integer(sc, 32));
      p = s7_symbol_local_value(sc, s7_make_symbol(sc, "new-env-var"), another_let);
      if ((!s7_is_integer(p)) || (s7_integer(p) != 32))
	{fprintf(stderr, "%d: second local new-env-var is %s?\n", __LINE__, s1 = TO_STR(p)); free(s1);}
      s7_set_curlet(sc, another_let);
      p = s7_slot(sc, s7_make_symbol(sc, "new-env-var"));
      if (s7_is_immutable(p))
	fprintf(stderr, "%d: second local new-env-var is immutable\n", __LINE__);
      s7_gc_unprotect_at(sc, another_loc);
      s7_set_curlet(sc, new_env);
    }

    s7_define_constant_with_environment(sc, s7_rootlet(sc), "new-env-var", s7_make_integer(sc, 321));
    if (!s7_is_defined(sc, "new-env-var"))
      {fprintf(stderr, "%d: new-env-var is not defined globally?\n", __LINE__);}
    p = s7_symbol_local_value(sc, s7_make_symbol(sc, "new-env-var"), new_env);
    if (s7_integer(p) != 123)
      {fprintf(stderr, "%d: local new-env-var: %s\n", __LINE__, s1 = TO_STR(p)); free(s1);}
    p = s7_symbol_local_value(sc, s7_make_symbol(sc, "new-env-var"), s7_rootlet(sc));
    if (s7_integer(p) != 321)
      {fprintf(stderr, "%d: global new-env-var: %s\n", __LINE__, s1 = TO_STR(p)); free(s1);}
    s7_set_curlet(sc, s7_rootlet(sc));
    p = s7_slot(sc, s7_make_symbol(sc, "new-env-var"));
    if (!s7_is_immutable(p))
      fprintf(stderr, "%d: global new-env-var in new_env is not immutable\n", __LINE__);
  ...
  {
    g_block *g;
    s7_pointer gp = g_make_block(sc, s7_list(sc, 1, TO_S7_INT(32)));
    ...
    if (g->size != 32)
      {fprintf(stderr, "%d: g_block length: %" ld64 " %s\n", __LINE__, g->size, s1 = TO_STR(gp)); free(s1);}
    ...
    {
      g_block *g1;
      s7_pointer gp1 = g_make_block(sc, s7_list(sc, 1, TO_S7_INT(2)));
      s7_int gc1 = s7_gc_protect(sc, gp1);
      g1 = (g_block *)s7_c_object_value(gp1);
      g->data[0] = 1.0;
      g1->data[0] = 2.0;
      s7_c_object_set_value(gp, g1);
      if (s7_c_object_value(gp) != s7_c_object_value(gp1))
	fprintf(stderr, "%d: s7_c_object_set_value: %p %p\n", __LINE__, s7_c_object_value(gp), s7_c_object_value(gp1));
      s7_c_object_set_value(gp, g); /* put it back the old way so GC won't try to hit data twice, normally you'd create the new g_block by itself */
      s7_gc_unprotect_at(sc, gc1);
    }
    s7_gc_unprotect_at(sc, gc_loc);
  }
...

  {
    ...
    if (s7_type_of(sc, p) != s7_make_symbol(sc, "random-state?"))
      {fprintf(stderr, "%d: s7_random_state returned %s\n", __LINE__, s1 = TO_STR(p)); free(s1);}
    if (strcmp(s7_string(s7_type_name(sc, p)), "a random-state") != 0)
      fprintf(stderr, "%d: type-name(vector) %s\n", __LINE__, s7_string(s7_type_name(sc, p)));
